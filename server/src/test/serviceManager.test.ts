...
describe("ServiceManager", () => {
    ...
    describe("validate order", () => {
        ...
        it("validateOrder returns true if the order object has valid SELL direction", () => {
            expect(serviceManager.validateOrder(new Order(AccountType.USD, 1, 1, DirectionType.Sell))).toBe(true);
        });
    
        it("validateOrder returns true if the order object has valid BUY direction", () => {
            expect(serviceManager.validateOrder(new Order(AccountType.USD, 1, 1, DirectionType.Buy))).toBe(true);
        });
    });
    ...
    describe("handleNewOrder", () => {
        ...
        it("adds the new order to the database if no matches are found", () => {
            const orders: Order[] = [];
            const newOrder = new Order(AccountType.USD, 10, 1, DirectionType.Buy);
        ...
        it("adds the new order to the database if it is partially completed", () => {
            ...
            const serviceManager = new ServiceManager(orders, [], createMatcher(orders));
            const newOrder = new Order(AccountType.USD, 30, 10, DirectionType.Buy);
            serviceManager.handleNewOrder(newOrder);
        ...
        it("does not add a new order to the database if it is fulfilled", () => {
            const orders = [
                new Order(AccountType.USD, 30, 10, DirectionType.Sell),
            ];

            const serviceManager = new ServiceManager(orders, [], createMatcher(orders));
            const newOrder = new Order(AccountType.USD, 30, 1, DirectionType.Buy);
            serviceManager.handleNewOrder(newOrder);
            expect(newOrder.quantity === 0 && orders.includes(newOrder)).toBe(false);
        });

        it("removes completed orders from the database", () => {
            const existingOrder = new Order(AccountType.USD, 20, 1, DirectionType.Sell);
            const orders = [
                existingOrder
            ];
        ...
        it("removes orders with 0 remaining quantity from the database", () => {
            const orders = [
                new Order(AccountType.USD, 20, 1, DirectionType.Sell),
                new Order(AccountType.USD, 20, 0, DirectionType.Sell),
                new Order(AccountType.USD, 20, 0, DirectionType.Sell),
                new Order(AccountType.USD, 20, 0, DirectionType.Sell),
            ];
